...
select min(popularity),min(height_m),min(weight_kg), min(intelligence),min(strength),min(speed),min(durability),min(energy_Projection),min(fighting_Skills) from marvels as 
'min_values';

select max(popularity),max(height_m),max(weight_kg), max(intelligence),max(strength),max(speed),max(durability),max(energy_Projection),max(fighting_Skills) from marvels as 'max_values';

select avg(popularity),avg(height_m),avg(weight_kg), avg(intelligence),avg(strength),avg(speed),avg(durability),avg(energy_Projection),avg(fighting_Skills) from marvels as 'avg_values';


select name,hometown,weight_kg from marvels group by hometown having weight_kg>100;

select *,
    CASE
        WHEN weight_kg>120 THEN 'Over weight'
        WHEN weight_kg>80 THEN 'Normal weight'
        WHEN weight_kg>50 THEN 'Common weight'
        Else 'under weight'
    END as 'weight_range'
from marvels;

SELECT NAME FROM marvels where gender='Female' and hometown='USA';